...
func Chatroom() {
	...
	for {
		select {
		case c := <-subscribe: //새로운 사용자가 들어왔을 떄
			var events []Event
			...
			c <- Subscription{events, subscriber} //구독 구조체 인스턴스를 생성하여
			//채널 c에 보냄

		case event := <-publish: // 새이벤트가 발행 됬을 떄
		...
		case c := <-unsubscribe: //사용자가 나갔을 때
			for e := subscribers.Front(); e != nil; e = e.Next() {
				...
				if subscriber == c { //구독자 목록에 들어 있는 이벤트와 채널 c가 같으면
...
func main() {
	server, err := socketio.NewServer(nil) // 소캣 초기화
	if err != nil {
		log.Fatal(err)
	}
	...
	//웹브라우저에서 socket.io로 접속했을 때 실행할 콜백 설정
	server.On("connection", func(so socketio.Socket) {
		...
		for _, event := range s.Archive { //지금까지 쌓인 이벤트를
			so.Emit("eent", event) //웹 브라우저로 접속한 사용자에게 보냄
		}

		newMessages := make(chan string)

		//웹브라우저에서 보내오는 채팅 메시지를 받을 수 있도록 콜백 설정
		so.On("message", func(msg string) {
			newMessages <- msg
		})

		//웹브라우저의 접속이 끊어졌을 때 콜백 설정
		...
		go func() {
			for {
				...
				case event := <-s.New: //채널에 이벤트가 들어오면
					so.Emit("event", event) //이벤트 데이터를 웹브라우저에 보냄
				case msg := <-newMessages: //웹브라우저에서 채팅 메시지를 보내오면
